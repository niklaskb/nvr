...
class FtpCamera(object):
    ...
    def _event_thread(self, event_timestamp):
        ...
        while current_image is None and datetime.now() < event_timestamp + timedelta(seconds=self._event_timeout_seconds):
            time.sleep(0.5)
            current_image = self._current_image(event_timestamp)

        if current_image is not None:
            new_filename = f"{event_timestamp.strftime('%Y%m%d_%H%M%S')}_{self._camera_name}"
            new_image_path = f"{self._image_file_path}{new_filename}.jpeg"
            self._logger.info(f"Found image {current_image}, moving to {new_image_path}")
            ...
            while matching_video is None and datetime.now() < event_timestamp + timedelta(seconds=self._video_timeout_seconds):
                time.sleep(1)
                matching_video = self._matching_video(current_image)
            
            if current_image is not None:
                new_video_path = f"{self._video_file_path}{new_filename}.mp4"
                self._logger.info(f"Found video {matching_video}, moving to {new_video_path}")
                os.rename(matching_video, new_video_path)
            else:
                self._logger.warn(f"Did not find a matching video file for {current_image}")
        else:
            self._logger.info(f"Did not find any image for event at {event_timestamp}")

    def _current_image(self, event_timestamp):
        earliest_timestamp = event_timestamp - timedelta(seconds=self._image_max_age_seconds)
    ...
    def _remove_old_files(self, event_timestamp):
        all_files = glob.iglob(self._ftp_upload_path + '**/**', recursive=True)
        image_and_video_files = list(filter(lambda x: x.endswith(".jpg") or x.endswith(".mp4"), all_files))
